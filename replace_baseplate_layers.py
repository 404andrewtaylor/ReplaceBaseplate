...
def extract_gcode_from_3mf(input_3mf, output_gcode):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        extract_cmd = ["unzip", "-q", input_3mf, "-d", str(temp_path)]
        result = subprocess.run(extract_cmd, capture_output=True, text=True)
        if result.returncode != 0:
            raise Exception(f"Error extracting 3MF: {result.stderr}")
...
def replace_gcode_in_3mf(input_3mf, modified_gcode, output_3mf):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if md5_file.exists():
            md5_cmd = ["md5", "-q", str(gcode_file)]
            result = subprocess.run(md5_cmd, capture_output=True, text=True)
            if result.returncode == 0:
                with open(md5_file, 'w') as f:
                    f.write(result.stdout.strip())
        
        # Repackage the 3MF file
        original_dir = os.getcwd()
        os.chdir(temp_path)
        zip_cmd = ["zip", "-r", output_3mf, ".", "-x", "*.DS_Store"]
        result = subprocess.run(zip_cmd, capture_output=True, text=True)
        os.chdir(original_dir)
        
        if result.returncode != 0:
            raise Exception(f"Error repackaging 3MF: {result.stderr}")
        
        # Move the file to the original directory
        temp_output = temp_path / output_3mf
        final_output = Path(input_3mf).parent / output_3mf
